...
class Visualizer3D:
    ...
    def plot_latest_session(self):
        """Fetch latest non-empty session data and open 3D Dashboard."""
        print("Generating Dashboard...")
        
        conn = self.db._get_connection()
        c = conn.cursor()
        
        # Get sessions ordered by most recent, skip empty ones
        ...
        for (candidate_table,) in all_rows:
            if not candidate_table:
                continue
            try:
                query = f"SELECT * FROM {candidate_table} ORDER BY timestamp ASC"
                candidate_df = pd.read_sql_query(query, conn)
        ...
        if df is None or df.empty:
            print("No sessions with recorded data found.")
            return

        print(f"Visualizing session: {table_name} ({len(df)} frames)")
